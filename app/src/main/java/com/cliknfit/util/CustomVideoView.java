...
import com.cliknfit.activity.Video;
...
/**
 * Created by prince on 21/09/17.
 */

public class CustomVideoView extends RelativeLayout implements SurfaceHolder.Callback, MediaPlayer.OnPreparedListener, MediaPlayer.OnErrorListener, MediaPlayer.OnSeekCompleteListener, MediaPlayer.OnCompletionListener, MediaPlayer.OnInfoListener, MediaPlayer.OnVideoSizeChangedListener {
    ...
    /**
     * Initializes all objects CustomVideoView depends on
     */
    ...
    /**
     * Releases all objects CustomVideoView depends on
     */
    ...
    protected void tryToPrepare() {
        if (this.surfaceIsReady && this.videoIsReady) {
          ...
          //  if (shouldAutoplay)
            videoplaceholder.setVisibility(GONE);
                start();
            ...
            new Handler().postDelayed(new Runnable() {
                ...
                public void run() {
                    stop();
                    onProgressView.setVisibility(GONE);
                }
            }, 500);
    ...
    /**
     * Get the current {@link CustomVideoView.State}.
     *
     * @return Current {@link CustomVideoView.State}
     */
    ...
    public void setFullscreen(final boolean fullscreen) throws RuntimeException {
        ...
        if (this.currentState != State.ERROR) {
            if (CustomVideoView.this.fullscreen == fullscreen) return;
            CustomVideoView.this.fullscreen = fullscreen;

            final boolean wasPlaying = mediaPlayer.isPlaying();
            if (wasPlaying)
                pause();

            if (CustomVideoView.this.fullscreen) {
                if (activity != null)
                ...
                if (viewParent instanceof ViewGroup) {
                    ...
                    currentLayoutParams = CustomVideoView.this.getLayoutParams();

                    parentView.removeView(CustomVideoView.this);
                } else
                    Log.e(TAG, "Parent View is not a ViewGroup");

                if (v instanceof ViewGroup) {
                    ((ViewGroup) v).addView(CustomVideoView.this);
                } else
                ...
                if (viewParent instanceof ViewGroup) {
                    ...
                    ((ViewGroup) viewParent).removeView(CustomVideoView.this);
                    if (parentHasParent) {
                        parentView.addView(CustomVideoView.this);
                        CustomVideoView.this.setLayoutParams(currentLayoutParams);
                    }
                }
            }
    ...
    public void setVideoPath(String path) throws IOException, IllegalStateException, SecurityException, IllegalArgumentException, RuntimeException {
        if (mediaPlayer != null) {
            if (currentState != State.IDLE)
                throw new IllegalStateException("CustomVideoView Invalid State: " + currentState);
